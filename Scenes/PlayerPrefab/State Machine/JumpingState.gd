...
func _walk(delta: float) -> Vector3: 
	...
	if move_dir != Vector2.ZERO:
		...
		if target_speed.length() <= walk_vel.length():
			walk_vel = walk_vel.move_toward(target_speed, acceleration * delta * air_friction)
		else:
			walk_vel = walk_vel.move_toward(target_speed, acceleration * delta * air_friction*6)
...
func check_transitions():
	if player.is_on_floor():
		if $"../../floorRayCast3D".is_colliding() and Input.is_action_pressed("crouch"):
		...
		if !(pressed_jump and first_jump) and !Input.is_action_pressed("sprint"):
			Transitioned.emit(self, 'Walking')
		if !pressed_jump and Input.is_action_pressed("sprint"):
