...
func check_transitions() -> void:
	...
	if player.is_on_floor_only() and $"../../floorRayCast3D".is_colliding() and Input.is_action_pressed("crouch"):
		...
		if Vector3.UP.dot(collision_normal) < 0 and Vector3.UP.dot(collision_normal) > -1 :
			Transitioned.emit(self, 'Sliding')

#TODO: Store standard transitions in a separate function
